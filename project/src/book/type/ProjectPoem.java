...
public class ProjectPoem extends JPanel {
	private DefaultTableModel model;
	private JTable table;
	private List<ProjectBookDTO> list;
	private JPanel poemP;
	private int num;

	public ProjectPoem(int num, String loginId) {
		this.num = num;
		setLayout(new BorderLayout());

		poemP = new JPanel();
		poemP.setLayout(null);

		// 목록생성
		Vector<String> poemV = new Vector<String>();
		poemV.addElement("번호");
		poemV.addElement("제목");
		poemV.addElement("저자");
		poemV.addElement("장르");
		poemV.addElement("출판사");
		poemV.addElement("가격");
		if (num == 1)
			poemV.add("권수");

		// 모델생성
		model = new DefaultTableModel(poemV, 0) {
		...
		List<ProjectBookDTO> allList = bookDAO.genreList("시");
		...
		poemP.add(scroll);
		poemP.setBackground(Color.WHITE);// 바탕색
		add(poemP);
	...
	public void search(int subjectCombobox, String searchT) {// 검색
		...
		if (subjectCombobox == 0) {// 제목별을 선택했을때
			...
			list = bookDAO.searchTypeBookName("시", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
			...
			List<ProjectBookDTO> list = new ArrayList<ProjectBookDTO>();
			list = bookDAO.searchTypeWriter("시", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
			...
			List<ProjectBookDTO> list = new ArrayList<ProjectBookDTO>();
			list = bookDAO.searchTypePublisher("시", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
