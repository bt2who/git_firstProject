...
public class ProjectNovel extends JPanel {// 소설
	...
	public ProjectNovel(int num, String loginId) {
		this.num = num;
		...
		// 목록생성
		Vector<String> novelV = new Vector<String>();
		novelV.addElement("번호");
		novelV.addElement("제목");
		novelV.addElement("저자");
		novelV.addElement("장르");
		novelV.addElement("출판사");
		novelV.addElement("가격");
		if (num == 1)
			novelV.add("권수");

		// 모델생성
		model = new DefaultTableModel(novelV, 0) {
		...
		List<ProjectBookDTO> allList = bookDAO.genreList("소설");
	...
	public void search(int subjectCombobox, String searchT) {// 검색
		...
		if (subjectCombobox == 0) {// 제목별을 선택했을때
			...
			list = bookDAO.searchTypeBookName("소설", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
			...
			List<ProjectBookDTO> list = new ArrayList<ProjectBookDTO>();
			list = bookDAO.searchTypeWriter("소설", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
			...
			List<ProjectBookDTO> list = new ArrayList<ProjectBookDTO>();
			list = bookDAO.searchTypePublisher("소설", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
