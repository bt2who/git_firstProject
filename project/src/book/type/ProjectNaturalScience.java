...
public class ProjectNaturalScience extends JPanel { // 자연과학
	...
	public ProjectNaturalScience(int num, String loginId) {
		this.num = num;
		...
		List<ProjectBookDTO> allList = bookDAO.genreList("자연과학");
		...
		nsP.add(scroll);
		nsP.setBackground(Color.WHITE);// 바탕색
		add(nsP);
	...
	public void search(int subjectCombobox, String searchT) {// 검색
		...
		if (subjectCombobox == 0) {// 제목별을 선택했을때
			...
			list = bookDAO.searchTypeBookName("자연과학", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
			...
			List<ProjectBookDTO> list = new ArrayList<ProjectBookDTO>();
			list = bookDAO.searchTypeWriter("자연과학", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
			...
			List<ProjectBookDTO> list = new ArrayList<ProjectBookDTO>();
			list = bookDAO.searchTypePublisher("자연과학", searchT); // searchT.getText();에 입력된 값을 제목에서 찾아라
