...
public class ProjectMemberFindId extends JFrame implements ActionListener {
	
	private JLabel nameL,mentL,emailL,codeL,logoSL;// 라벨
	private JLabel nameLT,  emailLT,codeLT; //라벨입력양식 안내 LT=LabelText 줄임
	private JTextField nameT,emailT,codeT; // 입력창
	private JButton findIdBtn,cancelBtn, emailCheckBtn,codeCheckBtn; //버튼
	private JComboBox emailComboBox; 
	private String returnCode2,returnFindId;  //지역을 벗어난 함수의 리턴값을 받기위해 이곳에 변수초기화
	private String email,email2,code; //code:여러지역에 변수가 중복으로 쓰여서 선언.
	//emial1,2:다른 지역에서 쓰인 값을 그대로 사용하기위해 선언

	// TODO id찾기 화면 창-----------------
	public ProjectMemberFindId() {
		
	setLayout(null); //버튼,라벨
		...
		nameL.setFont(font2);
		nameL.setBounds(50,120,130,25);
		nameT = new JTextField();
		nameT.setBounds(170,120,272,25);   
		nameLT = new JLabel("가입된 실명 입력");
		nameLT.setFont(font3);
		nameLT.setBounds(175,143,150,25);
		...
		emailCheckBtn = new JButton(new ImageIcon("insend.png"));
		...
		findIdBtn = new JButton(new ImageIcon("find_id.jpg"));
		findIdBtn.setBounds(90,320,152,40);
		...
		con.add(cancelBtn);
		//세팅
		setTitle("https://www.bit199.com/Templates/FTForgetID.aspx");
		setBounds(200, 150, 500, 440);
		setVisible(true);
		...
		findIdBtn.setEnabled(false);
		
		
		
	}
	// TODO 이벤트 창
	public void eventFindId() {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == emailCheckBtn) { //메일+이름 중복체크+코드보내기
			...
			returnFindId=memberDAO.findId(nameT.getText(),emailT.getText(),(String)emailComboBox.getSelectedItem());
			
			
			if(!returnFindId.equals("")) {
				...
				JOptionPane.showMessageDialog(this, "이름 혹은 이메일이 잘못 입력되었습니다.");
				findIdBtn.setEnabled(false);
...
if(e.getSource() == codeCheckBtn) {
			...
			if(code.equals(returnCode2)){
				JOptionPane.showMessageDialog(this, "메일 인증 완료. 아이디찾기 버튼을 누르세요.");
				
				findIdBtn.setEnabled(true); 
...
if(e.getSource() == findIdBtn) {
	...
	if(code.equals(returnCode2)){
		...
		new ProjectMemberEmail().emailSendId(returnFindId,email,email2);
		JOptionPane.showMessageDialog(this, "입력하신 메일주소로 가입된 아이디가 전송되었습니다.");
...
}////class MemberFindId extends JFrame implements ActionListener